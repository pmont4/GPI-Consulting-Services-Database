...
CREATE OR ALTER VIEW report.engineer_view
AS
	SELECT
...
SELECT ev.* FROM report.engineer_view ev ORDER BY ev.[ID engineer];

CREATE OR ALTER PROCEDURE report.plant_information
	@plant VARCHAR(200)
AS
	BEGIN TRY
		IF (@plant IS NOT NULL)
			BEGIN
				BEGIN
					SELECT * INTO #temp_plant FROM report.plant_table;
					SELECT * INTO #temp_report FROM report.report_table;

					CREATE CLUSTERED INDEX index_temp_plant ON #temp_plant(id_plant);
					CREATE CLUSTERED INDEX index_temp_report ON #temp_report(id_report);
				END;
				...
				BEGIN
					IF (TRY_CAST(@plant AS INT) IS NOT NULL)
						SET @id_plant = ISNULL((SELECT id_plant FROM #temp_plant WHERE id_plant = @plant), NULL);
					ELSE
						SET @id_plant = ISNULL((SELECT id_plant FROM #temp_plant WHERE plant_account_name = @plant OR plant_name = @plant), NULL);
				END;
				...
				IF (@id_plant IS NOT NULL)
					BEGIN
						...
						WHERE p.id_plant = @id_plant
							...
							lst.loss_scenario_total_insured_values, lst.loss_scenario_pml_percentage, lst.loss_scenario_mfl;
					END;
				ELSE IF (@id_plant IS NULL)
					PRINT 'Cannot bring data because the input table is null.';
			END;
		ELSE IF (@plant IS NULL)
			PRINT 'You must enter the plant name or id to bring data.';
		
		DROP TABLE #temp_plant;
