		...
		BEGIN
			...
			ELSE IF (LOWER(@filter) = 'riot risk' OR LOWER(@filter) = 'riesgo de huelga' OR LOWER(@filter) = 'riesgo de sindicato')
				BEGIN
					...
					END;
				END;
			ELSE IF (LOWER(@filter) = 'design failure risk' OR LOWER(@filter) = 'riesgo de fallas de dise�o')
				BEGIN
			...
			ELSE IF (LOWER(@filter) = 'buildings amount')
				BEGIN
					...
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						...
						END;
				END;
			ELSE IF (LOWER(@filter) = 'machinery and equipment amount' OR LOWER(@filter) = 'machinery amount')
				BEGIN
					IF (@param LIKE '%,%' AND @param LIKE '%:%' AND (@param LIKE '%rango%' OR @param LIKE '%range%'))
						BEGIN
							BEGIN TRY
								DECLARE
									@values_to_evaluate_machinary AS VARCHAR(100),
									@highiest_value_machinary AS INT,
									@lowest_value_machinary AS INT;

								DECLARE @value_range_machinary AS VARCHAR(100);
								DECLARE cur_range_machinary CURSOR DYNAMIC FORWARD_ONLY
															FOR SELECT * FROM STRING_SPLIT(@param, ',');
								OPEN cur_range_machinary;
								FETCH NEXT FROM cur_range_machinary INTO @value_range_machinary;
								WHILE @@FETCH_STATUS = 0
									BEGIN
										IF (@value_range_machinary LIKE '%:%')
											BEGIN
												IF (PATINDEX('%[0-9]%', @value_range_machinary) > 0)
													SET @values_to_evaluate_machinary = @value_range_machinary;
											END;
										FETCH NEXT FROM cur_range_machinary INTO @value_range_machinary
									END;
								CLOSE cur_range_machinary;
								DEALLOCATE cur_range_machinary;
							END TRY
							BEGIN CATCH
								CLOSE cur_range_machinary;
								DEALLOCATE cur_range_machinary;
							END CATCH;
							BEGIN
								IF (@values_to_evaluate_machinary IS NOT NULL)
									BEGIN TRY
										DECLARE @helper_machinary AS INT;
										DECLARE cur_evaluate_range_machinary CURSOR DYNAMIC FORWARD_ONLY
																				FOR SELECT * FROM STRING_SPLIT(@values_to_evaluate_machinary, ':');
										OPEN cur_evaluate_range_machinary;
										FETCH NEXT FROM cur_evaluate_range_machinary INTO @helper_machinary;
										WHILE @@FETCH_STATUS = 0
											BEGIN
												IF (@highiest_value_machinary IS NULL)
													SET @highiest_value_machinary = @helper_machinary;
											
												IF (@lowest_value_machinary IS NULL AND @highiest_value_machinary IS NOT NULL)
													IF (@helper_machinary < @highiest_value_machinary)
														SET @lowest_value_machinary = @helper_machinary;
													ELSE IF (@helper_machinary > @highiest_value_machinary)
														BEGIN
															DECLARE @temp_machinary AS INT = @highiest_value_machinary;
															SET @lowest_value_machinary = @temp_machinary;
															SET @highiest_value_machinary = @helper_machinary;
														END;
												FETCH NEXT FROM cur_evaluate_range_machinary INTO @helper_machinary
											END;
										CLOSE cur_evaluate_range_machinary;
										DEALLOCATE cur_evaluate_range_machinary;
									END TRY
									BEGIN CATCH
										CLOSE cur_evaluate_range_machinary;
										DEALLOCATE cur_evaluate_range_machinary;
							...
							BEGIN
								IF (@highiest_value_machinary IS NOT NULL AND @lowest_value_machinary IS NOT NULL)
									BEGIN
										IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_machinery_equipment_amount >= @lowest_value_machinary AND lst.loss_scenario_machinery_equipment_amount <= @highiest_value_machinary + 1) IS NOT NULL)
											BEGIN
												SELECT DISTINCT
													r.id_report AS 'ID report',
												...
												FROM #report_temp_table_filter r
													...
													LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
													LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
												WHERE lst.loss_scenario_machinery_equipment_amount >= @lowest_value_machinary AND lst.loss_scenario_machinery_equipment_amount <= @highiest_value_machinary + 1
											END;
										ELSE
											PRINT 'No values where found with that machinery and equipment amount filter';
									END;
								ELSE
									PRINT 'Cannot evaluate any data because the range of the machinery and equipment could not be determinated';
							END;
						END;
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						BEGIN TRY
							DECLARE 
								@evaluate_machinary AS VARCHAR(2),
								@amount_machinary AS INT;

							DECLARE @value_machinary AS VARCHAR(100);
							DECLARE cur_machinary CURSOR DYNAMIC FORWARD_ONLY
													FOR SELECT * FROM STRING_SPLIT(@param, ',');
							OPEN cur_machinary;
							FETCH NEXT FROM cur_machinary INTO @value_machinary;
							WHILE @@FETCH_STATUS = 0
								BEGIN
									IF (TRY_CAST(@value_machinary AS INT) IS NULL)
										BEGIN
											IF (TRIM(LOWER(@value_machinary)) = 'more than' OR TRIM(LOWER(@value_machinary)) = 'mas que' OR TRIM(LOWER(@value_machinary)) = 'mayor que'
												OR TRIM(LOWER(@value_machinary)) = 'less than' OR TRIM(LOWER(@value_machinary)) = 'menos que' OR TRIM(LOWER(@value_machinary)) = 'menor que')
												SET @evaluate_machinary = (SELECT CASE
																					WHEN TRIM(LOWER(@value_machinary)) = 'more than' OR TRIM(LOWER(@value_machinary)) = 'mas que' OR TRIM(LOWER(@value_machinary)) = 'mayor que' THEN '>'
																					WHEN TRIM(LOWER(@value_machinary)) = 'less than' OR TRIM(LOWER(@value_machinary)) = 'menos que' OR TRIM(LOWER(@value_machinary)) = 'menor que' THEN '<'
																				END);
										END;
									ELSE
										SET @amount_machinary = CAST(@value_machinary AS INT);
									FETCH NEXT FROM cur_machinary INTO @value_machinary;
								END;
							CLOSE cur_machinary;
							DEALLOCATE cur_machinary;
						END TRY
						BEGIN CATCH
							CLOSE cur_machinary;
							DEALLOCATE cur_machinary;
						...
						BEGIN
							IF (@evaluate_machinary IS NOT NULL AND @amount_machinary IS NOT NULL)
								BEGIN
									IF (@evaluate_machinary = '>')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_machinery_equipment_amount > @amount_machinary) IS NOT NULL)
												BEGIN
													SELECT DISTINCT
														r.id_report AS 'ID report',
													...
													FROM #report_temp_table_filter r
														...
														LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
														LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
													WHERE lst.loss_scenario_machinery_equipment_amount > @amount_machinary
												END;
											ELSE
												PRINT 'No values where found with that machinery and equipment amount filter';
										END;
									ELSE IF (@evaluate_machinary = '<')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_machinery_equipment_amount < @amount_machinary) IS NOT NULL)
												...
												END;
											ELSE
												PRINT 'No values where found with that machinery and equipment filter';
										END;
								END;
						END;
				END;
			ELSE IF (LOWER(@filter) = 'electronic equipment amount' OR LOWER(@filter) = 'electronic amount')
				BEGIN
					IF (@param LIKE '%,%' AND @param LIKE '%:%' AND (@param LIKE '%rango%' OR @param LIKE '%range%'))
						BEGIN
							BEGIN TRY
								DECLARE
									@values_to_evaluate_electronic AS VARCHAR(100),
									@highiest_value_electronic AS INT,
									@lowest_value_electronic AS INT;

								DECLARE @value_range_electronic AS VARCHAR(100);
								DECLARE cur_range_electronic CURSOR DYNAMIC FORWARD_ONLY
																FOR SELECT * FROM STRING_SPLIT(@param, ',');
								OPEN cur_range_electronic;
								FETCH NEXT FROM cur_range_electronic INTO @value_range_electronic;
								WHILE @@FETCH_STATUS = 0
									BEGIN
										IF (@value_range_electronic LIKE '%:%')
											BEGIN
												IF (PATINDEX('%[0-9]%', @value_range_electronic) > 0)
													SET @values_to_evaluate_electronic = @value_range_electronic;
											END;
										FETCH NEXT FROM cur_range_electronic INTO @value_range_electronic
									END;
								CLOSE cur_range_electronic;
								DEALLOCATE cur_range_electronic;
							END TRY
							BEGIN CATCH
								CLOSE cur_range_electronic;
								DEALLOCATE cur_range_electronic;
							END CATCH;
							BEGIN
								IF (@values_to_evaluate_electronic IS NOT NULL)
									BEGIN TRY
										DECLARE @helper_electronic AS INT;
										DECLARE cur_evaluate_range_electronic CURSOR DYNAMIC FORWARD_ONLY
																				FOR SELECT * FROM STRING_SPLIT(@values_to_evaluate_electronic, ':');
										OPEN cur_evaluate_range_electronic;
										FETCH NEXT FROM cur_evaluate_range_electronic INTO @helper_electronic;
										WHILE @@FETCH_STATUS = 0
											BEGIN
												IF (@highiest_value_electronic IS NULL)
													SET @highiest_value_electronic = @helper_electronic;
											
												IF (@lowest_value_electronic IS NULL AND @highiest_value_electronic IS NOT NULL)
													IF (@helper_electronic < @highiest_value_electronic)
														SET @lowest_value_electronic = @helper_electronic;
													ELSE IF (@helper_electronic > @highiest_value_electronic)
														BEGIN
															DECLARE @temp_electronic AS INT = @highiest_value_electronic;
															SET @lowest_value_electronic = @temp_electronic;
															SET @highiest_value_electronic = @helper_electronic;
														END;
												FETCH NEXT FROM cur_evaluate_range_electronic INTO @helper_electronic
											END;
										CLOSE cur_evaluate_range_electronic;
										DEALLOCATE cur_evaluate_range_electronic;
									END TRY
									BEGIN CATCH
										CLOSE cur_evaluate_range_electronic;
										DEALLOCATE cur_evaluate_range_electronic;
							...
							BEGIN
								IF (@highiest_value_electronic IS NOT NULL AND @lowest_value_electronic IS NOT NULL)
									BEGIN
										IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_electronic_equipment_amount >= @lowest_value_electronic AND lst.loss_scenario_electronic_equipment_amount <= @highiest_value_electronic + 1) IS NOT NULL)
											BEGIN
												SELECT DISTINCT
													r.id_report AS 'ID report',
												...
												FROM #report_temp_table_filter r
													...
													LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
													LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
												WHERE lst.loss_scenario_electronic_equipment_amount >= @lowest_value_electronic AND lst.loss_scenario_electronic_equipment_amount <= @highiest_value_electronic + 1
											END;
										ELSE
											PRINT 'No values where found with that electronic equipment amount filter';
									END;
								ELSE
									PRINT 'Cannot evaluate any data because the range of the electronic equipment could not be determinated';
							END;
						END;
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						BEGIN TRY
							DECLARE 
								@evaluate_electronic AS VARCHAR(2),
								@amount_electronic AS INT;

							DECLARE @value_electronic AS VARCHAR(100);
							DECLARE cur_electronic CURSOR DYNAMIC FORWARD_ONLY
													FOR SELECT * FROM STRING_SPLIT(@param, ',');
							OPEN cur_electronic;
							FETCH NEXT FROM cur_electronic INTO @value_electronic;
							WHILE @@FETCH_STATUS = 0
								BEGIN
									IF (TRY_CAST(@value_electronic AS INT) IS NULL)
										BEGIN
											IF (TRIM(LOWER(@value_electronic)) = 'more than' OR TRIM(LOWER(@value_electronic)) = 'mas que' OR TRIM(LOWER(@value_electronic)) = 'mayor que'
												OR TRIM(LOWER(@value_electronic)) = 'less than' OR TRIM(LOWER(@value_electronic)) = 'menos que' OR TRIM(LOWER(@value_electronic)) = 'menor que')
												SET @evaluate_electronic = (SELECT CASE
																					WHEN TRIM(LOWER(@value_electronic)) = 'more than' OR TRIM(LOWER(@value_electronic)) = 'mas que' OR TRIM(LOWER(@value_electronic)) = 'mayor que' THEN '>'
																					WHEN TRIM(LOWER(@value_electronic)) = 'less than' OR TRIM(LOWER(@value_electronic)) = 'menos que' OR TRIM(LOWER(@value_electronic)) = 'menor que' THEN '<'
																				END);
										END;
									ELSE
										SET @amount_electronic = CAST(@value_electronic AS INT);
									FETCH NEXT FROM cur_electronic INTO @value_electronic;
								END;
							CLOSE cur_electronic;
							DEALLOCATE cur_electronic;
						END TRY
						BEGIN CATCH
							CLOSE cur_electronic;
							DEALLOCATE cur_electronic;
						...
						BEGIN
							IF (@evaluate_electronic IS NOT NULL AND @amount_electronic IS NOT NULL)
								BEGIN
									IF (@evaluate_electronic = '>')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_electronic_equipment_amount > @amount_electronic) IS NOT NULL)
												BEGIN
													SELECT DISTINCT
														r.id_report AS 'ID report',
													...
													FROM #report_temp_table_filter r
														...
														LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
														LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
													WHERE lst.loss_scenario_electronic_equipment_amount > @amount_electronic
												END;
											ELSE
												PRINT 'No values where found with that electronic equipment amount filter';
										END;
									ELSE IF (@evaluate_electronic = '<')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_electronic_equipment_amount < @amount_electronic) IS NOT NULL)
												...
												END;
											ELSE
												PRINT 'No values where found with that electronic equipment filter';
										END;
								END;
						END;
				END;
			ELSE IF (LOWER(@filter) = 'expansion or investment amount' OR LOWER(@filter) = 'expansion amount' OR LOWER(@filter) = 'investment amount')
				BEGIN
					IF (@param LIKE '%,%' AND @param LIKE '%:%' AND (@param LIKE '%rango%' OR @param LIKE '%range%'))
						BEGIN
							BEGIN TRY
								DECLARE
									@values_to_evaluate_expansion AS VARCHAR(100),
									@highiest_value_expansion AS INT,
									@lowest_value_expansion AS INT;

								DECLARE @value_range_expansion AS VARCHAR(100);
								DECLARE cur_range_expansion CURSOR DYNAMIC FORWARD_ONLY
																FOR SELECT * FROM STRING_SPLIT(@param, ',');
								OPEN cur_range_expansion;
								FETCH NEXT FROM cur_range_expansion INTO @value_range_expansion;
								WHILE @@FETCH_STATUS = 0
									BEGIN
										IF (@value_range_expansion LIKE '%:%')
											BEGIN
												IF (PATINDEX('%[0-9]%', @value_range_expansion) > 0)
													SET @values_to_evaluate_expansion = @value_range_expansion;
											END;
										FETCH NEXT FROM cur_range_expansion INTO @value_range_expansion
									END;
								CLOSE cur_range_expansion;
								DEALLOCATE cur_range_expansion;
							END TRY
							BEGIN CATCH
								CLOSE cur_range_expansion;
								DEALLOCATE cur_range_expansion;
							END CATCH;
							BEGIN
								IF (@values_to_evaluate_expansion IS NOT NULL)
									BEGIN TRY
										DECLARE @helper_expansion AS INT;
										DECLARE cur_evaluate_range_expansion CURSOR DYNAMIC FORWARD_ONLY
																				FOR SELECT * FROM STRING_SPLIT(@values_to_evaluate_expansion, ':');
										OPEN cur_evaluate_range_expansion;
										FETCH NEXT FROM cur_evaluate_range_expansion INTO @helper_expansion;
										WHILE @@FETCH_STATUS = 0
											BEGIN
												IF (@highiest_value_expansion IS NULL)
													SET @highiest_value_expansion = @helper_expansion;
											
												IF (@lowest_value_expansion IS NULL AND @highiest_value_expansion IS NOT NULL)
													IF (@helper_expansion < @highiest_value_expansion)
														SET @lowest_value_expansion = @helper_expansion;
													ELSE IF (@helper_expansion > @highiest_value_expansion)
														BEGIN
															DECLARE @temp_expansion AS INT = @highiest_value_expansion;
															SET @lowest_value_expansion = @temp_expansion;
															SET @highiest_value_expansion = @helper_expansion;
														END;
												FETCH NEXT FROM cur_evaluate_range_expansion INTO @helper_expansion
											END;
										CLOSE cur_evaluate_range_expansion;
										DEALLOCATE cur_evaluate_range_expansion;
									END TRY
									BEGIN CATCH
										CLOSE cur_evaluate_range_expansion;
										DEALLOCATE cur_evaluate_range_expansion;
							...
							BEGIN
								IF (@highiest_value_expansion IS NOT NULL AND @lowest_value_expansion IS NOT NULL)
									BEGIN
										IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_expansions_investment_works_amount >= @lowest_value_expansion AND lst.loss_scenario_expansions_investment_works_amount <= @highiest_value_expansion + 1) IS NOT NULL)
											BEGIN
												SELECT DISTINCT
													r.id_report AS 'ID report',
												...
												FROM #report_temp_table_filter r
													...
													LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
													LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
												WHERE lst.loss_scenario_expansions_investment_works_amount >= @lowest_value_expansion AND lst.loss_scenario_expansions_investment_works_amount <= @highiest_value_expansion + 1
											END;
										ELSE
											PRINT 'No values where found with that expansions/investment amount filter';
									END;
								ELSE
									PRINT 'Cannot evaluate any data because the range of the expansions/investment amount could not be determinated';
							END;
						END;
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						BEGIN TRY
							DECLARE 
								@evaluate_expansion AS VARCHAR(2),
								@amount_expansion AS INT;

							DECLARE @value_expansion AS VARCHAR(100);
							DECLARE cur_expansion CURSOR DYNAMIC FORWARD_ONLY
													FOR SELECT * FROM STRING_SPLIT(@param, ',');
							OPEN cur_expansion;
							FETCH NEXT FROM cur_expansion INTO @value_expansion;
							WHILE @@FETCH_STATUS = 0
								BEGIN
									IF (TRY_CAST(@value_expansion AS INT) IS NULL)
										BEGIN
											IF (TRIM(LOWER(@value_expansion)) = 'more than' OR TRIM(LOWER(@value_expansion)) = 'mas que' OR TRIM(LOWER(@value_expansion)) = 'mayor que'
												OR TRIM(LOWER(@value_expansion)) = 'less than' OR TRIM(LOWER(@value_expansion)) = 'menos que' OR TRIM(LOWER(@value_expansion)) = 'menor que')
												SET @evaluate_expansion = (SELECT CASE
																					WHEN TRIM(LOWER(@value_expansion)) = 'more than' OR TRIM(LOWER(@value_expansion)) = 'mas que' OR TRIM(LOWER(@value_expansion)) = 'mayor que' THEN '>'
																					WHEN TRIM(LOWER(@value_expansion)) = 'less than' OR TRIM(LOWER(@value_expansion)) = 'menos que' OR TRIM(LOWER(@value_expansion)) = 'menor que' THEN '<'
																				END);
										END;
									ELSE
										SET @amount_expansion = CAST(@value_expansion AS INT);
									FETCH NEXT FROM cur_expansion INTO @value_expansion;
								END;
							CLOSE cur_expansion;
							DEALLOCATE cur_expansion;
						END TRY
						BEGIN CATCH
							CLOSE cur_expansion;
							DEALLOCATE cur_expansion;
						...
						BEGIN
							IF (@evaluate_expansion IS NOT NULL AND @amount_expansion IS NOT NULL)
								BEGIN
									IF (@evaluate_expansion = '>')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_expansions_investment_works_amount > @amount_expansion) IS NOT NULL)
												BEGIN
													SELECT DISTINCT
														r.id_report AS 'ID report',
													...
													FROM #report_temp_table_filter r
														...
														LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
														LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
													WHERE lst.loss_scenario_expansions_investment_works_amount > @amount_expansion
												END;
											ELSE
												PRINT 'No values where found with that expansion/investment amount filter';
										END;
									ELSE IF (@evaluate_expansion = '<')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_expansions_investment_works_amount < @amount_expansion) IS NOT NULL)
						...
						END;
				END;
			ELSE IF (LOWER(@filter) = 'inventory amount' OR LOWER(@filter) = 'stock amount')
				BEGIN
					IF (@param LIKE '%,%' AND @param LIKE '%:%' AND (@param LIKE '%rango%' OR @param LIKE '%range%'))
						BEGIN
							BEGIN TRY
								DECLARE
									@values_to_evaluate_stock AS VARCHAR(100),
									@highiest_value_stock AS INT,
									@lowest_value_stock AS INT;

								DECLARE @value_range_stock AS VARCHAR(100);
								DECLARE cur_range_stock CURSOR DYNAMIC FORWARD_ONLY
																FOR SELECT * FROM STRING_SPLIT(@param, ',');
								OPEN cur_range_stock;
								FETCH NEXT FROM cur_range_stock INTO @value_range_stock;
								WHILE @@FETCH_STATUS = 0
									BEGIN
										IF (@value_range_stock LIKE '%:%')
											BEGIN
												IF (PATINDEX('%[0-9]%', @value_range_stock) > 0)
													SET @values_to_evaluate_stock = @value_range_stock;
											END;
										FETCH NEXT FROM cur_range_stock INTO @value_range_stock
									END;
								CLOSE cur_range_stock;
								DEALLOCATE cur_range_stock;
							END TRY
							BEGIN CATCH
								CLOSE cur_range_stock;
								DEALLOCATE cur_range_stock;
							END CATCH;
							BEGIN
								IF (@values_to_evaluate_stock IS NOT NULL)
									BEGIN TRY
										DECLARE @helper_stock AS INT;
										DECLARE cur_evaluate_range_stock CURSOR DYNAMIC FORWARD_ONLY
																				FOR SELECT * FROM STRING_SPLIT(@values_to_evaluate_stock, ':');
										OPEN cur_evaluate_range_stock;
										FETCH NEXT FROM cur_evaluate_range_stock INTO @helper_stock;
										WHILE @@FETCH_STATUS = 0
											BEGIN
												IF (@highiest_value_stock IS NULL)
													SET @highiest_value_stock = @helper_stock;
											
												IF (@lowest_value_stock IS NULL AND @highiest_value_stock IS NOT NULL)
													IF (@helper_stock < @highiest_value_stock)
														SET @lowest_value_stock = @helper_stock;
													ELSE IF (@helper_stock > @highiest_value_stock)
														BEGIN
															DECLARE @temp_stock AS INT = @highiest_value_stock;
															SET @lowest_value_stock = @temp_stock;
															SET @highiest_value_stock = @helper_stock;
														END;
												FETCH NEXT FROM cur_evaluate_range_stock INTO @helper_stock
											END;
										CLOSE cur_evaluate_range_stock;
										DEALLOCATE cur_evaluate_range_stock;
									END TRY
									BEGIN CATCH
										CLOSE cur_evaluate_range_stock;
										DEALLOCATE cur_evaluate_range_stock;
							...
							BEGIN
								IF (@highiest_value_stock IS NOT NULL AND @lowest_value_stock IS NOT NULL)
									BEGIN
										IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_stock_amount >= @lowest_value_stock AND lst.loss_scenario_stock_amount <= @highiest_value_stock + 1) IS NOT NULL)
											BEGIN
												SELECT DISTINCT
													r.id_report AS 'ID report',
												...
												FROM #report_temp_table_filter r
													...
													LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
													LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
												WHERE lst.loss_scenario_stock_amount >= @lowest_value_stock AND lst.loss_scenario_stock_amount <= @highiest_value_stock + 1
											END;
										ELSE
											PRINT 'No values where found with that stock/inventory amount filter';
									END;
								ELSE
									PRINT 'Cannot evaluate any data because the range of the stock/inventory amount could not be determinated';
							END;
						END;
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						BEGIN TRY
							DECLARE 
								@evaluate_stock AS VARCHAR(2),
								@amount_stock AS INT;

							DECLARE @value_stock AS VARCHAR(100);
							DECLARE cur_stock CURSOR DYNAMIC FORWARD_ONLY
													FOR SELECT * FROM STRING_SPLIT(@param, ',');
							OPEN cur_stock;
							FETCH NEXT FROM cur_stock INTO @value_stock;
							WHILE @@FETCH_STATUS = 0
								BEGIN
									IF (TRY_CAST(@value_stock AS INT) IS NULL)
										BEGIN
											IF (TRIM(LOWER(@value_stock)) = 'more than' OR TRIM(LOWER(@value_stock)) = 'mas que' OR TRIM(LOWER(@value_stock)) = 'mayor que'
												OR TRIM(LOWER(@value_stock)) = 'less than' OR TRIM(LOWER(@value_stock)) = 'menos que' OR TRIM(LOWER(@value_stock)) = 'menor que')
												SET @evaluate_stock = (SELECT CASE
																				WHEN TRIM(LOWER(@value_stock)) = 'more than' OR TRIM(LOWER(@value_stock)) = 'mas que' OR TRIM(LOWER(@value_stock)) = 'mayor que' THEN '>'
																				WHEN TRIM(LOWER(@value_stock)) = 'less than' OR TRIM(LOWER(@value_stock)) = 'menos que' OR TRIM(LOWER(@value_stock)) = 'menor que' THEN '<'
																			END);
										END;
									ELSE
										SET @amount_stock = CAST(@value_stock AS INT);
									FETCH NEXT FROM cur_stock INTO @value_stock;
								END;
							CLOSE cur_stock;
						...
						BEGIN
							IF (@evaluate_stock IS NOT NULL AND @amount_stock IS NOT NULL)
								BEGIN
									IF (@evaluate_stock = '>')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_stock_amount > @amount_stock) IS NOT NULL)
												BEGIN
													SELECT DISTINCT
														r.id_report AS 'ID report',
													...
													FROM #report_temp_table_filter r
														...
														LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
														LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
													WHERE lst.loss_scenario_stock_amount > @amount_stock
												END;
											ELSE
												PRINT 'No values where found with that inventory/stock amount filter';
										END;
									ELSE IF (@evaluate_stock = '<')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_stock_amount < @amount_stock) IS NOT NULL)
						...
						END;
				END;
			ELSE IF (LOWER(@filter) = 'total values amount' OR LOWER(@filter) = 'total insured values amount')
				BEGIN
					IF (@param LIKE '%,%' AND @param LIKE '%:%' AND (@param LIKE '%rango%' OR @param LIKE '%range%'))
						BEGIN
							BEGIN TRY
								DECLARE
									@values_to_evaluate_total AS VARCHAR(100),
									@highiest_value_total AS INT,
									@lowest_value_total AS INT;

								DECLARE @value_range_total AS VARCHAR(100);
								DECLARE cur_range_total CURSOR DYNAMIC FORWARD_ONLY
															FOR SELECT * FROM STRING_SPLIT(@param, ',');
								OPEN cur_range_total;
								FETCH NEXT FROM cur_range_total INTO @value_range_total;
								WHILE @@FETCH_STATUS = 0
									BEGIN
										IF (@value_range_total LIKE '%:%')
											BEGIN
												IF (PATINDEX('%[0-9]%', @value_range_total) > 0)
													SET @values_to_evaluate_total = @value_range_total;
											END;
										FETCH NEXT FROM cur_range_total INTO @value_range_total
									END;
								CLOSE cur_range_total;
								DEALLOCATE cur_range_total;
							END TRY
							BEGIN CATCH
								CLOSE cur_range_total;
								DEALLOCATE cur_range_total;
							END CATCH;
							BEGIN
								IF (@values_to_evaluate_total IS NOT NULL)
									BEGIN TRY
										DECLARE @helper_total AS INT;
										DECLARE cur_evaluate_range_total CURSOR DYNAMIC FORWARD_ONLY
																			FOR SELECT * FROM STRING_SPLIT(@values_to_evaluate_total, ':');
										OPEN cur_evaluate_range_total;
										FETCH NEXT FROM cur_evaluate_range_total INTO @helper_total;
										WHILE @@FETCH_STATUS = 0
											BEGIN
												IF (@highiest_value_total IS NULL)
													SET @highiest_value_total = @helper_total;
											
												IF (@lowest_value_total IS NULL AND @highiest_value_total IS NOT NULL)
													IF (@helper_total < @highiest_value_total)
														SET @lowest_value_total = @helper_total;
													ELSE IF (@helper_total > @highiest_value_total)
														BEGIN
															DECLARE @temp_total AS INT = @highiest_value_total;
															SET @lowest_value_total = @temp_total;
															SET @highiest_value_total = @helper_total;
														END;
												FETCH NEXT FROM cur_evaluate_range_total INTO @helper_total
											END;
										CLOSE cur_evaluate_range_total;
										DEALLOCATE cur_evaluate_range_total;
									END TRY
									BEGIN CATCH
										CLOSE cur_evaluate_range_total;
										DEALLOCATE cur_evaluate_range_total;
							...
							BEGIN
								IF (@highiest_value_total IS NOT NULL AND @lowest_value_total IS NOT NULL)
									BEGIN
										IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_total_insured_values >= @lowest_value_total AND lst.loss_scenario_total_insured_values <= @highiest_value_total + 1) IS NOT NULL)
											BEGIN
												SELECT DISTINCT
													r.id_report AS 'ID report',
												...
												FROM #report_temp_table_filter r
													...
													LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
													LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
												WHERE lst.loss_scenario_total_insured_values >= @lowest_value_total AND lst.loss_scenario_total_insured_values <= @highiest_value_total + 1
											END;
										ELSE
											PRINT 'No values where found with that total insured values amount filter';
									END;
								ELSE
									PRINT 'Cannot evaluate any data because the range of the total insured values amount could not be determinated';
							END;
						END;
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						BEGIN TRY
							DECLARE 
								@evaluate_total AS VARCHAR(2),
								@amount_total AS INT;

							DECLARE @value_total AS VARCHAR(100);
							DECLARE cur_total CURSOR DYNAMIC FORWARD_ONLY
													FOR SELECT * FROM STRING_SPLIT(@param, ',');
							OPEN cur_total;
							FETCH NEXT FROM cur_total INTO @value_total;
							WHILE @@FETCH_STATUS = 0
								BEGIN
									IF (TRY_CAST(@value_total AS INT) IS NULL)
										BEGIN
											IF (TRIM(LOWER(@value_total)) = 'more than' OR TRIM(LOWER(@value_total)) = 'mas que' OR TRIM(LOWER(@value_total)) = 'mayor que'
												OR TRIM(LOWER(@value_total)) = 'less than' OR TRIM(LOWER(@value_total)) = 'menos que' OR TRIM(LOWER(@value_total)) = 'menor que')
												SET @evaluate_total = (SELECT CASE
																				WHEN TRIM(LOWER(@value_total)) = 'more than' OR TRIM(LOWER(@value_total)) = 'mas que' OR TRIM(LOWER(@value_total)) = 'mayor que' THEN '>'
																				WHEN TRIM(LOWER(@value_total)) = 'less than' OR TRIM(LOWER(@value_total)) = 'menos que' OR TRIM(LOWER(@value_total)) = 'menor que' THEN '<'
																			END);
										END;
									ELSE
										SET @amount_total = CAST(@value_total AS INT);
									FETCH NEXT FROM cur_total INTO @value_total;
								END;
							CLOSE cur_total;
						...
						BEGIN
							IF (@evaluate_total IS NOT NULL AND @amount_total IS NOT NULL)
								BEGIN
									IF (@evaluate_total = '>')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_total_insured_values > @amount_total) IS NOT NULL)
												BEGIN
													SELECT DISTINCT
														r.id_report AS 'ID report',
													...
													FROM #report_temp_table_filter r
														...
														LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
														LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
													WHERE lst.loss_scenario_total_insured_values > @amount_total
												END;
											ELSE
												PRINT 'No values where found with that total insured values amount filter';
										END;
									ELSE IF (@evaluate_total = '<')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_total_insured_values < @amount_total) IS NOT NULL)
						...
						END;
				END;
			ELSE IF (LOWER(@filter) = 'pml percentage' OR LOWER(@filter) = 'pml porcentaje' OR LOWER(@filter) = 'pml')
				BEGIN
					IF (@param LIKE '%,%' AND @param LIKE '%:%' AND (@param LIKE '%rango%' OR @param LIKE '%range%'))
						BEGIN
							BEGIN TRY
								DECLARE
									@values_to_evaluate_pml AS VARCHAR(100),
									@highiest_value_pml AS INT,
									@lowest_value_pml AS INT;

								DECLARE @value_range_pml AS VARCHAR(100);
								DECLARE cur_range_pml CURSOR DYNAMIC FORWARD_ONLY
															FOR SELECT * FROM STRING_SPLIT(@param, ',');
								OPEN cur_range_pml;
								FETCH NEXT FROM cur_range_pml INTO @value_range_pml;
								WHILE @@FETCH_STATUS = 0
									BEGIN
										IF (@value_range_pml LIKE '%:%')
											BEGIN
												IF (PATINDEX('%[0-9]%', @value_range_pml) > 0)
													SET @values_to_evaluate_pml = @value_range_pml;
											END;
										FETCH NEXT FROM cur_range_pml INTO @value_range_pml
									END;
								CLOSE cur_range_pml;
								DEALLOCATE cur_range_pml;
							END TRY
							BEGIN CATCH
								CLOSE cur_range_pml;
								DEALLOCATE cur_range_pml;
							END CATCH;
							BEGIN
								IF (@values_to_evaluate_pml IS NOT NULL)
									BEGIN TRY
										DECLARE @helper_pml AS INT;
										DECLARE cur_evaluate_range_pml CURSOR DYNAMIC FORWARD_ONLY
																			FOR SELECT * FROM STRING_SPLIT(@values_to_evaluate_pml, ':');
										OPEN cur_evaluate_range_pml;
										FETCH NEXT FROM cur_evaluate_range_pml INTO @helper_pml;
										WHILE @@FETCH_STATUS = 0
											BEGIN
												IF (@highiest_value_pml IS NULL)
													SET @highiest_value_pml = @helper_pml;
											
												IF (@lowest_value_pml IS NULL AND @highiest_value_pml IS NOT NULL)
													IF (@helper_pml < @highiest_value_pml)
														SET @lowest_value_pml = @helper_pml;
													ELSE IF (@helper_pml > @highiest_value_pml)
														BEGIN
															DECLARE @temp_pml AS INT = @highiest_value_pml;
															SET @lowest_value_pml = @temp_pml;
															SET @highiest_value_pml = @helper_pml;
														END;
												FETCH NEXT FROM cur_evaluate_range_pml INTO @helper_pml
											END;
										CLOSE cur_evaluate_range_pml;
										DEALLOCATE cur_evaluate_range_pml;
									END TRY
									BEGIN CATCH
										CLOSE cur_evaluate_range_pml;
										DEALLOCATE cur_evaluate_range_pml;
							...
							BEGIN
								IF (@highiest_value_pml IS NOT NULL AND @lowest_value_pml IS NOT NULL)
									BEGIN
										IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_pml_percentage >= @lowest_value_pml AND lst.loss_scenario_pml_percentage <= @highiest_value_pml + 1) IS NOT NULL)
											BEGIN
												SELECT DISTINCT
													r.id_report AS 'ID report',
												...
												FROM #report_temp_table_filter r
													...
													LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
													LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
												WHERE lst.loss_scenario_pml_percentage >= @lowest_value_pml AND lst.loss_scenario_pml_percentage <= @highiest_value_pml + 1
											END;
										ELSE
											PRINT 'No values where found with that pml percentage filter';
									END;
								ELSE
									PRINT 'Cannot evaluate any data because the range of the pml percentage could not be determinated';
							END;
						END;
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						BEGIN TRY
							DECLARE 
								@evaluate_pml AS VARCHAR(2),
								@amount_pml AS INT;

							DECLARE @value_pml AS VARCHAR(100);
							DECLARE cur_pml CURSOR DYNAMIC FORWARD_ONLY
													FOR SELECT * FROM STRING_SPLIT(@param, ',');
							OPEN cur_pml;
							FETCH NEXT FROM cur_pml INTO @value_pml;
							WHILE @@FETCH_STATUS = 0
								BEGIN
									IF (TRY_CAST(@value_pml AS INT) IS NULL)
										BEGIN
											IF (TRIM(LOWER(@value_pml)) = 'more than' OR TRIM(LOWER(@value_pml)) = 'mas que' OR TRIM(LOWER(@value_pml)) = 'mayor que'
												OR TRIM(LOWER(@value_pml)) = 'less than' OR TRIM(LOWER(@value_pml)) = 'menos que' OR TRIM(LOWER(@value_pml)) = 'menor que')
												SET @evaluate_pml = (SELECT CASE
																				WHEN TRIM(LOWER(@value_pml)) = 'more than' OR TRIM(LOWER(@value_pml)) = 'mas que' OR TRIM(LOWER(@value_pml)) = 'mayor que' THEN '>'
																				WHEN TRIM(LOWER(@value_pml)) = 'less than' OR TRIM(LOWER(@value_pml)) = 'menos que' OR TRIM(LOWER(@value_pml)) = 'menor que' THEN '<'
																			END);
										END;
									ELSE
										SET @amount_pml = CAST(@value_pml AS INT);
									FETCH NEXT FROM cur_pml INTO @value_pml;
								END;
							CLOSE cur_pml;
							DEALLOCATE cur_pml;
						...
						BEGIN
							IF (@evaluate_pml IS NOT NULL AND @amount_pml IS NOT NULL)
								BEGIN
									IF (@evaluate_pml = '>')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_pml_percentage > @amount_pml) IS NOT NULL)
												BEGIN
													SELECT DISTINCT
														r.id_report AS 'ID report',
													...
													FROM #report_temp_table_filter r
														...
														LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
														LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
													WHERE lst.loss_scenario_pml_percentage > @amount_pml
												END;
											ELSE
												PRINT 'No values where found with that pml percentage filter';
										END;
						...
						END;
				END;
			ELSE IF (LOWER(@filter) = 'mfl percentage' OR LOWER(@filter) = 'mfl porcentaje' OR LOWER(@filter) = 'mfl')
				BEGIN
					IF (@param LIKE '%,%' AND @param LIKE '%:%' AND (@param LIKE '%rango%' OR @param LIKE '%range%'))
						BEGIN
							BEGIN TRY
								DECLARE
									@values_to_evaluate_mfl AS VARCHAR(100),
									@highiest_value_mfl AS INT,
									@lowest_value_mfl AS INT;

								DECLARE @value_range_mfl AS VARCHAR(100);
								DECLARE cur_range_mfl CURSOR DYNAMIC FORWARD_ONLY
															FOR SELECT * FROM STRING_SPLIT(@param, ',');
								OPEN cur_range_mfl;
							...
							END CATCH;
							BEGIN
								IF (@values_to_evaluate_mfl IS NOT NULL)
									BEGIN TRY
										DECLARE @helper_mfl AS INT;
										DECLARE cur_evaluate_range_mfl CURSOR DYNAMIC FORWARD_ONLY
																			FOR SELECT * FROM STRING_SPLIT(@values_to_evaluate_mfl, ':');
										OPEN cur_evaluate_range_mfl;
										FETCH NEXT FROM cur_evaluate_range_mfl INTO @helper_mfl;
										WHILE @@FETCH_STATUS = 0
											BEGIN
												IF (@highiest_value_mfl IS NULL)
													SET @highiest_value_mfl = @helper_mfl;
											
												IF (@lowest_value_mfl IS NULL AND @highiest_value_mfl IS NOT NULL)
													IF (@helper_mfl < @highiest_value_mfl)
														SET @lowest_value_mfl = @helper_mfl;
													ELSE IF (@helper_mfl > @highiest_value_mfl)
														BEGIN
															DECLARE @temp_mfl AS INT = @highiest_value_mfl;
															SET @lowest_value_mfl = @temp_mfl;
															SET @highiest_value_mfl = @helper_mfl;
														END;
												FETCH NEXT FROM cur_evaluate_range_mfl INTO @helper_mfl
											END;
										CLOSE cur_evaluate_range_mfl;
										DEALLOCATE cur_evaluate_range_mfl;
									END TRY
									BEGIN CATCH
										CLOSE cur_evaluate_range_mfl;
										DEALLOCATE cur_evaluate_range_mfl;
							...
							BEGIN
								IF (@highiest_value_mfl IS NOT NULL AND @lowest_value_mfl IS NOT NULL)
									BEGIN
										IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_mfl >= @lowest_value_mfl AND lst.loss_scenario_mfl <= @highiest_value_mfl + 1) IS NOT NULL)
											BEGIN
												SELECT DISTINCT
													r.id_report AS 'ID report',
												...
												FROM #report_temp_table_filter r
													...
													LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
													LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
												WHERE lst.loss_scenario_mfl >= @lowest_value_mfl AND lst.loss_scenario_mfl <= @highiest_value_mfl + 1
											END;
										ELSE
											PRINT 'No values where found with that mfl percentage filter';
									END;
								ELSE
									PRINT 'Cannot evaluate any data because the range of the mfl percentage could not be determinated';
							END;
						END;
					ELSE IF (@param LIKE '%,%' AND ((@param NOT LIKE '%rango%' OR @param NOT LIKE '%range%') AND @param NOT LIKE '%:%'))
						BEGIN TRY
							DECLARE 
								@evaluate_mfl AS VARCHAR(2),
								@amount_mfl AS INT;

							DECLARE @value_mfl AS VARCHAR(100);
							...
							WHILE @@FETCH_STATUS = 0
								BEGIN
									IF (TRY_CAST(@value_mfl AS INT) IS NULL)
										BEGIN
											IF (TRIM(LOWER(@value_mfl)) = 'more than' OR TRIM(LOWER(@value_mfl)) = 'mas que' OR TRIM(LOWER(@value_mfl)) = 'mayor que'
												OR TRIM(LOWER(@value_mfl)) = 'less than' OR TRIM(LOWER(@value_mfl)) = 'menos que' OR TRIM(LOWER(@value_mfl)) = 'menor que')
												SET @evaluate_mfl = (SELECT CASE
																				WHEN TRIM(LOWER(@value_mfl)) = 'more than' OR TRIM(LOWER(@value_mfl)) = 'mas que' OR TRIM(LOWER(@value_mfl)) = 'mayor que' THEN '>'
																				WHEN TRIM(LOWER(@value_mfl)) = 'less than' OR TRIM(LOWER(@value_mfl)) = 'menos que' OR TRIM(LOWER(@value_mfl)) = 'menor que' THEN '<'
																			END);
										END;
									ELSE
										SET @amount_mfl = CAST(@value_mfl AS INT);
									FETCH NEXT FROM cur_mfl INTO @value_mfl;
								END;
							CLOSE cur_mfl;
							DEALLOCATE cur_mfl;
						...
						BEGIN
							IF (@evaluate_mfl IS NOT NULL AND @amount_mfl IS NOT NULL)
								BEGIN
									IF (@evaluate_mfl = '>')
										BEGIN
											IF ((SELECT TOP 1 r.id_report FROM #report_temp_table_filter r
																			LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
																			WHERE lst.loss_scenario_mfl > @amount_mfl) IS NOT NULL)
												BEGIN
													SELECT DISTINCT
														r.id_report AS 'ID report',
													...
													FROM #report_temp_table_filter r
														...
														LEFT JOIN report.perils_and_risk_table pr ON r.id_report = pr.id_report
														LEFT JOIN report.loss_scenario_table lst ON r.id_report = lst.id_report
													WHERE lst.loss_scenario_mfl > @amount_mfl
												END;
											ELSE
												PRINT 'No values where found with that mfl percentage filter';
										END;
...
EXEC report.reports_filter_by 'machinery amount', 'menos que,2000000';
EXEC report.reports_filter_by 'machinery amount', 'mas que,2000000';
EXEC report.reports_filter_by 'machinery amount', 'rango,2000000:6000000';

EXEC report.reports_filter_by 'electronic equipment amount', 'mas que,2000000';
EXEC report.reports_filter_by 'electronic equipment amount', 'menos que,2000000';
EXEC report.reports_filter_by 'electronic equipment amount', 'rango,200000:500000';

EXEC report.reports_filter_by 'expansion amount', 'mas que,100000';
EXEC report.reports_filter_by 'expansion amount', 'mas que,1000000';
EXEC report.reports_filter_by 'expansion amount', 'rango,3000000:5000000';

EXEC report.reports_filter_by 'stock amount', 'mas que,5000000';
EXEC report.reports_filter_by 'stock amount', 'menos que,5000000';
EXEC report.reports_filter_by 'stock amount', 'rango,5000000:10000000';

EXEC report.reports_filter_by 'total values amount', 'mas que,50000000';
EXEC report.reports_filter_by 'total values amount', 'menos que,80000000';
EXEC report.reports_filter_by 'total values amount', 'rango,80000000:100000000';
