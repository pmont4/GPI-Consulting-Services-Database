						...
						WHILE @@FETCH_STATUS = 0
							BEGIN
								IF (TRY_CAST(@value AS INT) IS NULL)
									BEGIN
										IF (LOWER(@value) = 'more than' OR LOWER(@value) = 'mas que'
											OR LOWER(@value) = 'less than' OR LOWER(@value) = 'menos que')
											SET @evaluate = (SELECT CASE
																		WHEN LOWER(@value) = 'more than' OR LOWER(@value) = 'mas que' THEN '>'
																		WHEN LOWER(@value) = 'less than' OR LOWER(@value) = 'menos que' THEN '<'
																	END);
										ELSE
...
EXEC report.reports_filter_by 'id report', '2032'
